...
function addselectchange(obj){
	var riskAddress = $("#addriskAddressSelect option:selected").text();//选择风险地点的select下拉框 当前选中的值
...
function deleteBtn(){
	$.ajax({
		type : "post",
		dataType : "json",
		url : "${pageContext.request.contextPath}/identify_delIdentifyRiskMsg.action",
...
function findAllRiRespon() {
	$.ajax({
		type : "post",
		dataType : "json",
		url : "${pageContext.request.contextPath}/identify_findIdentifyByCondition.action",
		data : {
			// 当前页页号 每页显示的记录数
		...
		success : function(data) {
			...
			for (var i = 0; i < data.pageBean.productList.length; i++) {
				...
				options +="<td><a data-toggle='modal' data-target='#modifierDuty' onclick='updateOpenBtn(this)' value='"+riskmsgid+"'>修改</a><a href='#' data-toggle='modal' data-target='#deleteRisk' onclick='delOpenBtn(this)'  value='"+riskmsgid+"'>删除</a></td>";
				options += "</tr>";
			}
